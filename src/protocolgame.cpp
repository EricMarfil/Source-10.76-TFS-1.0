...
void ProtocolGame::login(const std::string& name_, uint32_t accountId, OperatingSystem_t operatingSystem, bool gamemasterLogin)
{
	//dispatcher thread
	//uint32_t place = name_.find("-");
	std::string name = name_;
	/*if (place != std::string::npos)
	{
		name = name_.substr(0, place);
	}
	else name = name_;*/

	Player* _player = g_game.getPlayerByName(name);
	if (accountId == 0 && !_player)
	{
		disconnectClient(0x14, "This Cast has expired.");
		return;
	}
	...
	if (!original)
		...
		case 0x99:{
					  ...
					  if (this->player->cast.mCastChannelId == channelId)
						  this->sendCreatePrivateChannel(this->player->cast.mCastChannelId, "Spectators");
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	int32_t baloon = creature->getBaloon();
	if (baloon < 0 || baloon > 4)
	msg.AddByte(0); // baloons npcs
	else
	msg.AddByte(baloon); // baloons npcs
	}
	else
	msg.AddByte(0);
	*/
