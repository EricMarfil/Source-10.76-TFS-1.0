...
void Player::setCastDescription(std::string desc) {
	cast.description = desc;
}

const std::string& Player::getCastDescription() {
...
uint32_t Player::getCastViewerCount() {
	uint32_t count = 0;
	for (AutoList<ProtocolGame>::iterator it = cSpectators.begin(); it != cSpectators.end(); ++it)
	{
...
bool Player::isCastBanned(uint32_t ip)
{
	for (auto it : cast.bans)
	{
		if (it.ip == ip)
		{
			return true;
...
bool Player::isCastMutted(uint32_t ip)
{
	for (auto it : cast.muted)
	{
		if (it.ip == ip)
		{
			return true;
...
bool Player::addCastBan(std::string n) {
...
bool Player::removeCastBan(std::string n) {
...
bool Player::addCastMute(std::string n) {
	uint32_t ip = getCastIpByName(n);
	if (!ip)
		return false;
...
bool Player::removeCastMute(std::string n) {
...
void Player::dropLoot(Container* corpse, Creature* _lastHitCreature)
{
	...
	if (!inventory[CONST_SLOT_BACKPACK]) {
		__internalAddThing(CONST_SLOT_BACKPACK, Item::CreateItem(1988));
	}
}
