...
void Game::spectatorSay(const std::string& text, ProtocolGame * spectator)
{
	...
	if (player->isCastMutted(spectator->getIP()))
		return;
...
{
	if (effect > CONST_ANI_LAST || effect == CONST_ANI_UNK1 || effect == CONST_ANI_UNK2 || effect == CONST_ANI_UNK3 || effect == CONST_ANI_UNK4) {
		return;
	}
...
{
	playerSafeAccess.lock();
	const std::string& lowercase_name = asLowerCaseString(player->getName());
	mappedPlayerNames[lowercase_name] = player;
	wildcardTree.insert(lowercase_name);
	players[player->getID()] = player;
	playerSafeAccess.unlock();
}
