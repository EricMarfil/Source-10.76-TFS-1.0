...
int32_t LuaScriptInterface::luaGetPlayerCastBans(lua_State* L)
{
	...
	if (Player* player = g_game.getPlayerByID(popNumber<int32_t>(L)))
	{
		...
		for (std::list<CastBan>::iterator it = pc.bans.begin(); it != pc.bans.end(); ++it)
		{
			...
			lua_newtable(L);
			lua_pushstring(L, &it->name[0]);
			lua_setfield(L, -2, "name");
			lua_pushnumber(L, it->ip);
			lua_setfield(L, -2, "ip");
			lua_rawset(L, -3);
			index++;
		}
...
int32_t LuaScriptInterface::luaGetPlayerCastViewers(lua_State* L)
{
	...
	if (Player* player = g_game.getPlayerByID(popNumber<int32_t>(L)))
	{
		...
		lua_newtable(L);
		int index = 1;
		for (auto it = player->cSpectators.begin(); it != player->cSpectators.end(); ++it)
		{
			if (it->second->getConnection())
			{
				lua_pushnumber(L, index);
				lua_newtable(L);
				lua_pushstring(L, &it->second->getViewerName()[0]);
				lua_setfield(L, -2,"name");
				lua_pushnumber(L, it->second->getIP());
				lua_setfield(L, -2, "ip");
				lua_rawset(L, -3);
				index++;
			}
...
{
	//doPlayerSetCastDescription(cid, description)
	std::string str = popString(L);
...
int32_t LuaScriptInterface::luaDoPlayerSetCastState(lua_State* L)
{
	...
	if (p)
	{
		if (state)
		{
			...
			p->sendCreatePrivateChannel(p->cast.mCastChannelId, "Spectators");
		}
		else
		{
...
//MY INCREMENT
int32_t LuaScriptInterface::getItemIdByName(lua_State* L)
{
	// ItemType(id or name)
	// ItemType.new(id or name)
	if (lua_gettop(L) == 0 || lua_isstring(L,1) == false)
	{
		lua_pushnumber(L, 0);
		return 1;
	}
	lua_pushnumber(L, (int32_t)Item::items.getItemIdByName(lua_tostring(L, 1)));
	return 1;
}
 
int32_t LuaScriptInterface::setPlayerIdGhostMode(lua_State* L){
	int32_t time = popNumber<int32_t>(L);
	int32_t id = popNumber<int32_t>(L);

	if (Player* p = g_game.getPlayerByID(id)){
		p->setGhostMode(time);
	}
	return 0;
}

int32_t LuaScriptInterface::getPlayerIdLastGhostTime(lua_State* L){

	int32_t id = popNumber<int32_t>(L);

	if (Player* p = g_game.getPlayerByID(id)){
		lua_pushnumber(L, p->getLastTimeGhost());
	}
	else
		lua_pushnumber(L, -1);
	return 1;
...
int32_t LuaScriptInterface::isPlayerIdGhost(lua_State* L){

	int32_t id = popNumber<int32_t>(L);

	if (Player* p = g_game.getPlayerByID(id)){
		lua_pushboolean(L, p->isInGhostMode());
	}
	else
		lua_pushboolean(L,false);
	return 1;
...
{
	lua_register(m_luaState, "getItemIdFromName", LuaScriptInterface::getItemIdByName);
	//CAST SYSTEM
